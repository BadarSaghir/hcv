...
function downLoadAccountsJs() {
    if (window.RDL.Portal && window.RDL.Portal.useAccountsJs) {
        // create guest user
...
function handleClaims(result, resolve) {
    ...
    if (RDL.UserClaims.user_uid != null && RDL.UserClaims.user_uid != '' && navigator.cookieEnabled) {
    ...
    else if (window.RDL.Portal && window.RDL.Portal.useAccountsJs) {
        ...
        if (guestUserId == null || guestUserId == "" || window.RDL.readCookie(BoldAuthCookieName) == null) {
            if (window.RDL.Portal && window.RDL.Portal.isThirdPartySite) {
                ...
                if (isAccUserCalled && (RDL.Portal.useAccountsJs || RDL.isINTLAccounts)) {
                    // redirect to LP.
...
function checkBrowserCompatibility() {
    ...
    //     }
    // }
    if (!(window.RDL && window.RDL.LowerVersionBroswer)) {
...
function handleLocalizationText(result, resolve) {
    ...
    if (window.RDL.isINTL && !window.RDL.isFR) {
    ...
    RDL.localizationResumeRenderer = {
        ...
        "address_Label": window.RDL.Localization.address_Label || '',
        "email_Label": window.RDL.Localization.email_Label || '',
        "phone_Label": window.RDL.Localization.phone_Label || ''
    }
...
function handlePostPageLoad() {
    ...
    if(RDL.isINTL){
        var ecomPrefetchURL = 'https://' + window.location.host + '/payment/scripts/mpintlPrefetchScript.js';
        loadJs(ecomPrefetchURL);
    }
    ...
    if (RDL.sendSpellCheckText && RDL.clientEventsUrl) {
        (function () {
            loadJs(RDL.clientEventsUrl + "/scripts/boldEventStream.js", true, function () {
                var parentHtmlNode = (document.head == null ? document.body : document.head);
                var configObject = BoldEventStream.ClassInitializers.getNewConfigurationObject();
...
function setExperimentLocalizationObject() {
    ...
    if (RDL.PortalExperiments.mprSkillByJobTitleAndDesc) {
        RDL.ExperimentsLocalization[RDL.PortalExperiments.mprSkillByJobTitleAndDesc.id] = {
            "topSixSkill_Text": "We found 6 top skills for"
        }
    }   
}
...
function handleConfig(result, resolve) {
    ...
    RDL.PortalSettings.ConfigureClientCd = data.clientCD;
    ...
    // window.RDL.configServiceBlobUrl = "https://configservicestoragetest.blob.core.windows.net/livecareerrepository/";
    ...
    //Additional features
    window.RDL.shouldShowNotification = data.shouldShowNotification;
    window.RDL.shouldShowSMSPopup = data.shouldShowSMSPopup;
    //

    //Need to remove this after this is config driven for all portals 
    if (window.RDL.isINTL) {
        window.RDL.Definition_Tips = data.definition_tips;
        window.RDL.intlPhotoSkins = "ATA1 MTA3 MCA2 MLU4 MLU6 MLU5 MLU7 MLF1 MLF2 MLF3 MLF4 MLF5 MLF6 MLI1 MLJ1 MLJ2 MLJ3 MLJ4 MLJ5 MLJ6 MLJ7 MLI6 MLK1 MLK3 MLK5 MLK6 MLK7 MLK8";
        window.RDL.roundPhotoSkins = "MLJ2 MLJ5 MLK7";
    ...
    window.RDL.templatesOrderForFilter = data.templatesOrderForFilter;
    ...
    window.RDL.sampleDenominator = data.sampleDenominator;
...
function createGuestUser() {
    ...
    if (!RDL.isINTL || RDL.isINTLAccounts) {
        if (typeof BOLD == "object" && typeof BOLD.Accounts.createGuest == 'function' && window.RDL.pageLoaded) {
...
function getPortalInfo() {
    ...
    switch (domain) {
        ...
        case "myperfectcv.co.uk":
            ...
            RDL.isINTLAccounts = false;
             if(window.RDL.readCookie('lp') == 'MPRUKZLP02'){
                useAccountsJs =true;
                RDL.isINTLAccounts=true;
             }
            break;
    ...
    return {
        ...
        rewriteBlobContent: rewriteBlobContent
    };
}
...
RDL.GetElementPosition = function (element) {
    return typeof window.jQuery == "function" ? $(element).offset() : "";
}
...
RDL.promiseAllResolveActivity = function () {
    var _jobTitle = window.RDL.GetQueryString("JobTitle");
    if (_jobTitle) {
...
RDL.isAnyAuthCookieExists = function () {
    ...
    if (RDL.Portal.useAccountsJs || RDL.isINTLAccounts) {
